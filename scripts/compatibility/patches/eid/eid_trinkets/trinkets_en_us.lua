...
trinketTable = {
	...
	[Trinket.ALMAGEST_SCRAP.ID] = {
		...
		_modifier = function(descObj, desc)
			if not Mod:HasBitFlags(descObj.ObjSubType, TrinketType.TRINKET_GOLDEN_FLAG) then
				return desc
			end
		end,
		Name = "Almagest Scrap",
		Description = {
			...
			function(descObj)
				return trinketTable[Trinket.ALMAGEST_SCRAP.ID]._modifier(descObj,
	...
	[Trinket.LEAHS_LOCK.ID] = {
		...
		Description = {
			...
			function(descObj)
				return trinketTable[Trinket.LEAHS_LOCK.ID]._modifier(descObj,
				"#"
			)
			end
		}
	...
	[Trinket.LEVIATHANS_TENDRIL.ID] = {
		_modifier = function(descObj, desc, leviathanLine)
			...
			local multi = FR_EID:TrinketMulti(EID.player, descObj.ObjSubType)
			local chance1 = Trinket.LEVIATHANS_TENDRIL.REFLECT_CHANCE
			local chance2 = Trinket.LEVIATHANS_TENDRIL.FEAR_CHANCE
		...
		Description = function(descObj)
			return trinketTable[Trinket.LEVIATHANS_TENDRIL.ID]._modifier(descObj, {
