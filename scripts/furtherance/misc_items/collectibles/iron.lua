...
function Mod:WellCache(player, flag)
	if flag == CacheFlag.CACHE_FAMILIARS then
		player:CheckFamiliar(IronOrbital, player:GetCollectibleNum(CollectibleType.COLLECTIBLE_IRON),
			player:GetCollectibleRNG(CollectibleType.COLLECTIBLE_IRON),
			Isaac.GetItemConfig():GetCollectible(CollectibleType.COLLECTIBLE_IRON))
	end
end
