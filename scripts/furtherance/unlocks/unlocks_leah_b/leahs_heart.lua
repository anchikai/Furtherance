...
LEAHS_HEART.DAMAGE_MULT_UP = 1.2
...
function LEAHS_HEART:HeartDamage(player)
	...
	if player:HasCollectible(LEAHS_HEART.ID) and not player_floor_save.LeahsHeartUsedActive then
		player.Damage = player.Damage * LEAHS_HEART.DAMAGE_MULT_UP * player:GetCollectibleNum(LEAHS_HEART.ID) * Mod:GetPlayerDamageMultiplier(player)
	end
end
