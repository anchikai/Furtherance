	...
	if entity.Variant == Settings.VariantID then
		...
		elseif data.state == States.Unmask then
			...
			elseif data.state == States.Attacking then
				...
				if sprite:GetOverlayFrame() == 9 then
					entity:FireProjectiles(entity.Position,
						(target.Position - entity.Position):Normalized() * Settings.ShotSpeed, 0, ProjectileParams())
		...
		if (entity:HasMortalDamage() or entity:IsDead()) and entity.SubType ~= Settings.UnmaskID then
			...
			if entity.SubType == Settings.RealID then
				local real = Isaac.Spawn(entity.Type, entity.Variant, Settings.UnmaskID, entity.Position, entity
				.Velocity, nil)
				...
				-- Chain gibs
				for i = 0, 7 do
					Isaac.Spawn(EntityType.ENTITY_EFFECT, EffectVariant.CHAIN_GIB, 0, entity.Position,
						Vector(math.random(-4, 4), math.random(-4, 4)), entity)
				end

				-- Clone fear visuals
			elseif entity.SubType == Settings.CloneID then
				local smoke = Isaac.Spawn(EntityType.ENTITY_EFFECT, EffectVariant.FART, 2, entity.Position, Vector.Zero,
					entity):GetSprite()
				smoke.Color = Color(0.6, 0.25, 0.6, 1, 0.25, 0.1, 0.25)
				SFXManager():Stop(SoundEffect.SOUND_FART)

				local poof = Isaac.Spawn(EntityType.ENTITY_EFFECT, EffectVariant.POOF02, 1, entity.Position, Vector.Zero,
					entity):GetSprite()
