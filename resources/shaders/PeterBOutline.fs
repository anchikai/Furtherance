...
void main(void)
{
	...
	if (testfragColor.a > 0.05)
	{
		float AAA = testfragColor.a * 0.25;
		//fragColor = vec4(AAA, AAA, AAA, AAA);
		fragColor = vec4(testfragColor.r * 1.2, testfragColor.g * 1.2, testfragColor.b * 1.2, AAA + 0.5);
	}
	else
	{
		...
		//if (testfragColor.a < 0.05) {
			for(i = -1; i <= 1; i++) {
					for (j = -1; j <= 1; j++) {
						vec4 col = Color0 * texture(Texture0, TexCoord0 + vec2(i, j) * 1.0 / TextureSizeOut);
						//col.a = Color.a;
						if (col.a > 0.05) {
							//fragColor.rgb = vec3(1.0, 1.0, 1.0);
							//fragColor.a = 1.0;
							fragColor = vec4(0.5, 0.5, 0.5, 0.2) * col.a;
						}
					}
			}
		//}
