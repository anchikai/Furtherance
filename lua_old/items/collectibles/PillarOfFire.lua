...
function mod:SpawnFire(entity)
	...
	if (player and player:HasCollectible(CollectibleType.COLLECTIBLE_PILLAR_OF_FIRE)) then
		...
		if rollFire <= (player.Luck * 5) + 5 then
			local speed = 4
			for i = 1, 5 do
				local RandomVector1, RandomVector2
				...
				local fire = Isaac.Spawn(EntityType.ENTITY_EFFECT, EffectVariant.HOT_BOMB_FIRE, 0, player.Position,
					Vector(RandomVector1, RandomVector2), player)
